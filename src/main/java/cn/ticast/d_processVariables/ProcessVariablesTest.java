...
//        taskService.setVariable(taskId,"请假原因", "回家探亲，一起吃个饭");
        ...
        /**
         * 当一个javabean（实现序列号）放置当流程变量中，要求javabean属性不能再
         * 发生变化，如果发生变化，再获取的时候，抛出异常
         *
         * 解决方案：在Person对象中添加
...
//        System.out.println("请假原因："+resean);
    ...
    /**模拟设置和获取流程变量的场景*/
    public void serAndGetVariables(){
        /**与流程实例，执行对象（正在执行）*/
        ...
        /**设置流程变量*/
//        runtimeService.setVariable(executionId, variablename, value);//表示使用执行对象ID，和流程变量的名称，设置流程变量的值（一次只能设置一个）
//        runtimeService.setVariables(executionId, variables);//表示使用执行对象，和Map集合设置流程变量，map集合的key就是流程变量的名称，map集合的value就是流程变量的值。

//        taskService.setVariable(taskID, variablename, value);//表示使用任务ID，和流程变量的名称，设置流程变量的值（一次只能设置一个）
//        taskService.setVariables(taskId, variables);//表示使用任务对象，和Map集合设置流程变量，map集合的key就是流程变量的名称，map集合的value就是流程变量的值。
...
//        runtimeService.getVariables(executionId);//使用执行对象ID，获取所有的流程变量，将流程变量放置到Map集合中，和Map集合设置流程变量，map集合的key就是流程变量的名称，map集合的value就是流程变量的值。
//        runtimeService.getVariables(executionId, variableNames);//使用执行对象ID,获取流程变量的值，通过设置流程变量的名称存放到集合中，获取指定流程变量的名称的流程变量的值，值存放到Map集合?

//        runtimeService.getVariable(executionId, variableName);//使用任务ID和流程变量的名称，获取流程变量的值
//        runtimeService.getVariables(executionId);//使用任务ID，获取所有的流程变量，将流程变量放置到Map集合中，和Map集合设置流程变量，map集合的key就是流程变量的名称，map集合的value就是流程变量的值。
//        runtimeService.getVariables(executionId, variableNames);//使用任务ID,获取流程变量的值，通过设置流程变量的名称存放到集合中，获取指定流程变量的名称的流程变量的值，值存放到Map集合?
    }
